...
public class GraphingCalculator {
    ...
    public GraphingCalculator(int width, int height) {
        ...
        canvas.add(zoomIn);
        canvas.add(zoomOut);
        zoomIn.setCenter(40,10);
        zoomIn.onClick(() -> setScale(getScale() * 1.5));
        zoomOut.setCenter(200,10);
        zoomOut.onClick(()->setScale(getScale()*(2.0/3)));
//        Boolean animating=true;
        canvas.onMouseDown(event-> { animating=false;});
        canvas.onMouseUp(event -> {animating=true;});
        canvas.animate(() ->{
            if (animating){setAnimationParameter(
        ...
        canvas.onDrag(event ->{
                ...
                animationSpeed=(event.getDelta().getX() / width+getAnimationParameter()+animationSpeed)*0.5;
        });
