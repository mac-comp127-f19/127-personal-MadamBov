...
public class Library {
    ...
    public List<String> findTitlesByAuthor(String authorName) {
       ...
       result=books.stream().filter(d->d.getAuthor().equals(authorName)).map(Book::getTitle).collect(toList());
        return result;
    }
    ...
    public List<String> findTitlesAndAuthorsInGenre(String genre) {
        return books.stream().filter(d->d.getGenres().contains(genre)).map(d->d.getTitle()+"by"+d.getAuthor()).collect(toList());
    }
    ...
    public long countBooksInGenre(String genre) {
        long count;
        count=books.stream().filter(d->d.getGenres().contains(genre)).count();
        return count;
    }
